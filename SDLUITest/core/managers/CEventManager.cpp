...
void CEventManager::PollEvents()
{
	...
	while (SDL_PollEvent(&this->_Event))
	{
		...
		if (this->_Event.type == SDL_WINDOWEVENT)
		{
			...
			{
				CLog::debug("Window shown");
				break;
			}
			case SDL_WINDOWEVENT_HIDDEN:
			{
				CLog::debug("Window hidden");
				break;
			}
			case SDL_WINDOWEVENT_RESIZED:
			{
				CLog::debug("Window resized");
				break;
			}
			case SDL_WINDOWEVENT_SIZE_CHANGED:
			{
				CLog::debug("Window size changed");
				WindowManager->Resize(this->_Event.window.data1, this->_Event.window.data2);
				break;
			}
			case SDL_WINDOWEVENT_MOVED:
			{
				WindowManager->OnWindowMove();
				break;
			}
			default:
				break;
...
void CEventManager::GetMouseMotion(int& Mx, int& My)
{
	Mx = this->_MouseX;
	My = this->_MouseY;
}

bool CEventManager::GetIsRunning()
{
	return _IsRunning;
}

SDL_MouseButtonEvent CEventManager::GetMouseData()
{
	return MouseEvent;
}

array<bool, 322> CEventManager::GetKeyboardData()
{
	return this->Keys;
}
